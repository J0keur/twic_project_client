...
/**
 * Servlet implementation class InfoVilleController
 */
@WebServlet("/InfoVilleController")
public class InfoVilleController2 extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public InfoVilleController2() {
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			json = readJsonFromUrl("http://localhost:8182/villebyinsee?insee=" + insee);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("send").equals("Supprimer la ville")) {
			URL url = new URL("http://localhost:8182/ville?insee=" + request.getParameter("insee"));
			HttpURLConnection con = (HttpURLConnection) url.openConnection();
			con.setRequestMethod("DELETE");
			if (con.getResponseCode() == 200) {
				this.getServletContext().getRequestDispatcher("/WEB-INF/villeSuppr.jsp").forward(request, response);
			}
			...
			newVille.setCoordonnees(ancienneVille.getCoordonnees());
			newVille.setCodeInsee(ancienneVille.getCodeInsee());
			switch (choix) {
			case "Code Postal":
				newVille.setCodePostal(valeur);
				newVille.setLibelle(ancienneVille.getLibelle());
				newVille.setLigne5(ancienneVille.getLigne5());
			...
			case "Ligne":
				newVille.setCodePostal(ancienneVille.getCodePostal());
				newVille.setLibelle(ancienneVille.getLibelle());
				newVille.setLigne5(valeur);
				newVille.setNom(ancienneVille.getNom());
				break;
			}
			ObjectMapper mapper = new ObjectMapper();
			String jsonString = mapper.writeValueAsString(newVille);
			URL url = new URL("http://localhost:8182/villebyinsee?insee=" + request.getParameter("insee"));
			HttpURLConnection con = (HttpURLConnection) url.openConnection();
